"""
Generate submission.csv using XGBoost + CodeT5 Pipeline
...
4. Runs Stage 2 (CodeT5 multi-class classification) on AI samples
...
def generate_submission_codet5(
    ...
    """
    print("\n" + "="*70)
    print("GENERATING SUBMISSION: XGBoost + CodeT5 Pipeline")
    print("="*70 + "\n")
    ...
    # ================================================================
    # STEP 5: Load Stage 2 Classifier (CodeT5)
    # ================================================================
    print("\nSTEP 5/6: Loading Stage 2 CodeT5 classifier...")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Generate submission using XGBoost + CodeT5 pipeline',
...
Examples:
  ...
  uv run python scripts/generate_submission_codet5.py --test-file custom/test.parquet

  # Save to custom location
  uv run python scripts/generate_submission_codet5.py --output custom/submission.csv
        """
    )
    ...
    stage2_path = Path(args.models_dir) / 'stage2_codet5' / 'classifier.pth'
    ...
    if not stage2_path.exists():
        print(f"❌ ERROR: Stage 2 CodeT5 model not found: {stage2_path}")
        print("\nPlease train Stage 2 first:")
        print("  uv run python src/training/train_stage2_codet5.py")
        sys.exit(1)

    try:
        generate_submission_codet5(args.test_file, args.models_dir, args.output)
